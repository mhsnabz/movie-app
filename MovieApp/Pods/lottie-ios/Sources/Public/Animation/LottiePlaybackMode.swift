...
public extension LottiePlaybackMode {
    static var paused: Self {
        .paused(at: .currentFrame)
    }

    @available(*, deprecated, renamed: "LottiePlaybackMode.playing(_:)", message: "Will be removed in a future major release.")
    static func toProgress(_ toProgress: AnimationProgressTime, loopMode: LottieLoopMode) -> LottiePlaybackMode {
        .playing(.fromProgress(nil, toProgress: toProgress, loopMode: loopMode))
    }

    @available(*, deprecated, renamed: "LottiePlaybackMode.playing(_:)", message: "Will be removed in a future major release.")
    static func toFrame(_ toFrame: AnimationFrameTime, loopMode: LottieLoopMode) -> LottiePlaybackMode {
        .playing(.fromFrame(nil, toFrame: toFrame, loopMode: loopMode))
    }

    @available(*, deprecated, renamed: "LottiePlaybackMode.playing(_:)", message: "Will be removed in a future major release.")
    static func toMarker(
        _ toMarker: String,
        playEndMarkerFrame: Bool = true,
        loopMode: LottieLoopMode
    )
...
public extension LottiePlaybackMode.PlaybackMode {
    /// Plays the animation from the current progress to a progress value (0-1).
    /// - Parameter toProgress: The end progress of the animation.
    /// - Parameter loopMode: The loop behavior of the animation.
    static func toProgress(_ toProgress: AnimationProgressTime, loopMode: LottieLoopMode) -> Self {
    ...
    /// - Parameter toFrame: The end frame of the animation.
    /// - Parameter loopMode: The loop behavior of the animation.
    static func toFrame(_ toFrame: AnimationFrameTime, loopMode: LottieLoopMode) -> Self {
