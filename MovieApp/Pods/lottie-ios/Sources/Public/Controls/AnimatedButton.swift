...
open class AnimatedButton: AnimatedControl {
    ...
    #elseif canImport(AppKit)
        override open func handle(_ event: LottieNSControlEvent) {
            super.handle(event)
    ...
    private var rangesForEvents: [AnyHashable: (from: CGFloat, to: CGFloat)] = [LottieControlEvent.touchUpInside.id: (from: 0, to: 1)]
}
